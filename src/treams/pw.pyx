...
cdef double complex _cto_cw(double kzcw, long m, long polcw, number_t kx, number_t ky, number_t kzpw, long polpw) nogil:
...
cdef void _loop_cw_D(char **args, np.npy_intp *dims, np.npy_intp *steps, void *data) nogil:
    ...
    cdef double complex ov0
    for i in range(n):
        ov0 = (<double complex(*)(double, long, long, double complex, double complex, double complex, long) nogil>func)(
            <double>(<double*>ip0)[0],
            <long>(<long*>ip1)[0],
            <long>(<long*>ip2)[0],
            <double complex>(<double complex*>ip3)[0],
...
ufunc_cw_types[11] = <char>np.NPY_CDOUBLE
ufunc_cw_types[12] = <char>np.NPY_CDOUBLE
ufunc_cw_types[13] = <char>np.NPY_CDOUBLE
